...
import { mockElections, mockPollingStations } from "@/lib/mock-data";
import Link from "next/link";
import { Vote, Search, MapPin, Calendar, Users, Filter } from "lucide-react";
...
export default function ElectionsPage() {
  ...
  const filtered = mockElections.filter(e => {
    const matchSearch = e.title.toLowerCase().includes(search.toLowerCase()) || e.state.toLowerCase().includes(search.toLowerCase());
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <div className="relative flex-1">
            ...
            <Input className="pl-10" placeholder="Search elections..." value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
                    ...
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <Badge className={
                          ...
                          {e.status === "ongoing" && <span className="w-1.5 h-1.5 rounded-full bg-green-500 mr-1.5 animate-pulse inline-block" />}
                          {e.status.toUpperCase()}
                        </Badge>
                      </div>
                      <p className="text-muted-foreground text-sm mt-1">{e.description}</p>
                      <div className="flex flex-wrap gap-4 mt-2 text-sm text-muted-foreground">
                        <span className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5" />{e.state}</span>
                        <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5" />{e.startDate} → {e.endDate}</span>
                        <span className="flex items-center gap-1"><Users className="w-3.5 h-3.5" />{e.totalVoters.toLocaleString()} voters</span>
                      </div>
                      {e.status === "ongoing" && (
                        <div className="mt-3 max-w-xs">
                          ...
                          <div className="w-full bg-muted rounded-full h-2">
                            <div className="bg-primary h-2 rounded-full" style={{ width: `${(e.votescast / e.totalVoters) * 100}%` }} />
                          </div>
                        </div>
                      )}
                    </div>
              ...
              <Card key={ps.id} className="border border-border/50 hover:shadow-md transition-all">
                <CardContent className="p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <Badge variant="outline" className="text-xs">{ps.status === "active" ? "Normal" : "Issue"}</Badge>
                  </div>
                  <div className="font-medium text-sm">{ps.name}</div>
                  <div className="text-xs text-muted-foreground mt-1">{ps.district}</div>
                  <div className="text-xs text-muted-foreground">Booth #{ps.boothNumber} · {ps.observers} observers</div>
                  <Button size="sm" variant="outline" className="w-full mt-3 text-xs" onClick={() => {}}>
                    Get Directions
                  </Button>
                </CardContent>
              </Card>
            ...
            <CardContent className="p-0">
              <div className="h-80 rounded-xl bg-gradient-to-br from-green-50 to-blue-50 dark:from-green-900/20 dark:to-blue-900/20 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground">Polling station locations would display here with Leaflet.js</p>
